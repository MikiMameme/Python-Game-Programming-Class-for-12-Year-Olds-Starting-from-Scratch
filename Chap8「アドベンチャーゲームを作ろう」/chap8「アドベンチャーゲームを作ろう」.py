...
def decode_line(event):
    ...
    if line[0] != "#":
        ...
        if params[2] == "L":
            ...
            canvas.delete("right")
            lcharimg = tkinter.PhotoImage(file=params[1])
            canvas.create_image(700, 160, image=lcharimg, tag="right")
        else:
            canvas.delete("center")
            lcharimg = tkinter.PhotoImage(file=params[1])
            canvas.create_image(450, 160, image=lcharimg, tag="center")
...
#ファイル読み込み
scenario = []
file = open("12歳からはじめるゼロからのPythonゲームプログラミング教室/img8/scenario.txt", "r", encoding="utf-8")
while True:
    line = file.readline()
